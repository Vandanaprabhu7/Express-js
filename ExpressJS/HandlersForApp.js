...
app.use((req, res, next) => {
  ...
  next(); //mandatory for getting into next module
});
app.use((req, res, next) => {
...
//middleware for accessing static content, from public folder
...
// app.get("/api/v1/employees", getAllEmp);
// app.post("/api/v1/employees", addEmp);
// app.get("/api/v1/employees/:empId/:name?/", getEmpbyId);
// app.delete("/api/v1/employees/:id", deleteById);

//get and post have same url, if the v1 has to be changed to version 2, everywhere we need to make changes, rather ->
app.route("/api/v1/employees").get(getAllEmp).post(addEmp);
app.route("/api/v1/employees/:id").get(getEmpbyId).delete(deleteById);
